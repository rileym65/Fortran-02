...
void cpause(char* line) {
  ...
  Asm("          sep   scall                   ; Get key");
  Asm("          dw    f_read");
  Asm("          ori   32                      ; convert UC to LC");
  Asm("          smi   't'                     ; check for t");
  sprintf(buffer,"          lbnz  lbl_%d                  ; jump if not",nextLabel);
  Asm(buffer);
  if (useStg) {
    Asm("            ldi  STG_.1");
    Asm("            phi  rf");
  ...
  sprintf(buffer, "lbl_%d:",nextLabel++); Asm(buffer);
  }
