...
void crewind(char* line) {
  ...
  if (*line >= '0' && *line <= '9') {
    ...
    if (varType(v) == 'I') {
      sprintf(buffer,"              ldi   (v_%s+3).1        ; Point variable",
        variables[v].name);
      Asm(buffer);
      Asm("              phi   rf");
      sprintf(buffer,"              ldi   (v_%s+3).0",
        variables[v].name);
      Asm(buffer);
      Asm("              plo   rf");
      }
    else if (varType(v) == 'S') {
      sprintf(buffer,"              ldi   (v_%s+1).1        ; Point variable",
        variables[v].name);
      Asm(buffer);
      Asm("              phi   rf");
      sprintf(buffer,"              ldi   (v_%s+1).0",
        variables[v].name);
      Asm(buffer);
      Asm("              plo   rf");
      }
    else {
      sprintf(buffer,"              ldi   (v_%s).1          ; Point variable",
        variables[v].name);
      Asm(buffer);
      Asm("              phi   rf");
